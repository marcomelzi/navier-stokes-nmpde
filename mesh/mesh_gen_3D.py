...
# ==============================================================================
W = 0.41 # [m] Width of the channel (along X-axis)
H = 0.41 # [m] Height of the channel (along Y-axis)
L = 2.5  # [m] Length of the channel (along Z-axis)

y_cylinder = 0.20 # [m] y-coordinate of the cylinder center
z_cylinder = 0.50 # [m] z-coordinate of the cylinder center
...
channel = gmsh.model.occ.addBox(0, 0, 0, W, H, L)

# b) Create the cylinder
cylinder = gmsh.model.occ.addCylinder(0, y_cylinder, z_cylinder, W, 0, 0, R)
...
for dim, tag in surfaces:
    ...
    if abs(z_c - 0.0) < 1e-4:          # Surface at z = 0 is the Inflow
        inlet_surfaces.append(tag)
    elif abs(z_c - L) < 1e-4:          # Surface at z = L is the Outflow
        outlet_surfaces.append(tag)
    elif abs(y_c - 0.0) < 1e-4 or abs(y_c - H) < 1e-4 or abs(x_c - 0.0) < 1e-4 or abs(x_c - W) < 1e-4:
...
gmsh.model.addPhysicalGroup(2, cylinder_surfaces, name="Cylinder")


# Volume Group
gmsh.model.addPhysicalGroup(3, [fluid_domain_tag], name="Domain")
